...
exports.tests = {
    ...
    '[contract] should parse generic constructors': '{% contract Test = table.name: Text[A] %}',
    '[contract] should parse record types': "{% contract Test[A] =\n         \n        table.data.record: { \n\n          name: String,\n          table.name: Text[A],\n          table.data.list: A[] \n          } \n        %}",
    '[contract] should parse list types': "{% contract Test[A] = table.data.list: A[] %}",
    '[contract] should parse 2d list types': "{% contract Test[A,B] = table.data.list2: A[][] %}",
    '[contract] should parse 3d list types': "{% contract Test[A, B, C] = table.data.list3: A[][][] %}",
    '[contract] should parse func with no args or parens': "{% contract Test = value: Test -> Number %}",
    '[contract] should parse func with no args': "{% contract Test = value: () -> Number %}",
    '[contract] should parse no-parens func with constructor': "{% contract Test = value: String -> String %}",
    '[contract] should parse no-parens func with generic constructor': "{% contract Test = value: Text[A] -> Text[A] %}",
    '[contract] should parse no-parens func with record arg': "{% contract Test = value: {} -> { } %}",
    '[contract] should parse no-parens func with list arg': "{% contract Test = value: String[] -> String[] %}",
    '[contract] should parse func with cons arg': "{% contract Test = value: (String) -> String %}",
    '[contract] should parse func with 2 cons args': "{% contract Test = value: (String, String) -> String %}",
    '[contract] should parse func with 3 cons args': "{% contract Test = value: (String, String, String) -> String %}",
    '[contract] should parse func with generic cons arg': "{% contract Test[A] = value: (Text[A]) -> Text[A] %}",
    '[contract] should parse func with 2 generic cons args': "{% contract Test[A] = value: (Text[A], Text[A]) -> Text[A] %}",
    '[contract] should parse func with 3 generic cons args': "{% contract Test[A] = value: (Text[A], Text[A], Text[A]) -> Text[A] %}",
    '[contract] should parse func with record arg': "{% contract Test = value: ({ }) -> { } %}",
    '[contract] should parse func with 2 record args': "{% contract Test = value: ({ }, { name: String }) -> { } %}",
    '[contract] should parse func with 3 record args': "{% contract Test = value: ({ }, { name: String }, { value: A[]}) -> { } %}",
    '[contract] should parse func with list arg': "{% contract Test = value: (String[]) -> String[] %}",
    '[contract] should parse func with 2 list args': "{% contract Test = value: (String[], String[]) -> String[] %}",
    '[contract] should parse func with 3 list args': "{% contract Test = value: (String[], String[], String[]) -> String[] %}",
    '[contract] should parse func with func arg': "{% contract Test = value: (String -> String) -> String %}",
    '[contract] should parse func with 2 func args': "{% contract Test = value: ((String -> String), (String -> String)) -> String %}",
    '[contract] should parse func with 3 func args': "{% contract Test =\n          value: ((String -> String), (String -> String), (String -> String)) -> String \n         %}",
    '[contract] should parse func that return array of generic type': "{% contract Test[A] = value: Number -> Text[A][] %}",
    '[contract] should parse funct that return array of array': "{% contract Test = value: String -> Number[][] %}",
    ...
    '[contract] should mark nested properties as optional if all are': "{% contract Paper = object.type?: String %}",
    'should parse alias statements': "{% alias Type = String | Number | Boolean | Type[] | Type -> Type %}",
    'should parse tupe types': "{% alias Tuple = [Number, String, Number] %}",
    'should parse primitives used as type': '{% alias Prims = 1|2|"three"|true|false|"1,2,three,true,false" %}',
    'should parse this alias': '{% alias SortDir = 1 | -1 %}',
    ...
    '[instance] should parse instance statements': "{% instance head of HeadCtx %}\n     {% instance head2 of HeadCtx title = \"My Title\" %}\n    ",
    '[instance] should be usable in a view': "{% view MyView(Object) %}\n      {% instance head of HeadViewContext title = \"My Title\" %}\n      <h1>{{<HeadView(head)>}}</h1>\n    "
};
